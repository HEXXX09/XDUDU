...
本版本同时完成产品名从 XYCLI 到 XDUDU 的迁移。新名称的数据源优先，旧配置和会话仅作为兼容读取来源。凭据存储沿用原 XYCLI 的原生 `keyring` 实现，固定服务名改为 `xdudu`，不进行隐式跨服务迁移；用户通过 `auth login` 和 `auth logout` 显式管理凭据生命周期。
...
本文记录 v0.4.0 完成时确定的后续方向。该方向已经由 v0.5.0 M5 落地：SQLite、跨进程一致性、会话查询/恢复和上下文压缩均已实现，详细设计见 `M5_SESSION_RECOVERY_DESIGN.md`。
